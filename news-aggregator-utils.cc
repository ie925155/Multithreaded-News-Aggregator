 ...
 * by news-aggregator-utils.h. (Note that the implementation
...
{
  size_t start = 0;
  if (url.find(kHTTPPrefix) != string::npos)
    start = kHTTPPrefix.size();
  else if (url.find(kHTTPSPrefix) != string::npos)
    start = kHTTPSPrefix.size();
